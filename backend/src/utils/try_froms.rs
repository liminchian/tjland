//! https://github.com/rust-awesome-app/template-app-base/blob/main/src-tauri/src/model/store/try_froms.rs
//! 檢查值是否為特定型別，來決定是否可以進行 `try_from` 轉換
...
impl TryFrom<W<Value>> for Datetime {
    type Error = Error;
    fn try_from(value: W<Value>) -> Result<Self, Self::Error> {
        match value.0 {
